...
Either way, `.github/workflows/release-firmware.yml` builds and publishes the release — it rejects the run if the tag doesn't match `FIRMWARE_VERSION`. That workflow assembles `src/config.h` from repo secrets (`TZ_INFO`, `OWM_API_KEY`, and — for now, see below — `WIFI_SSID`/`WIFI_PASS`) rather than the CI job's placeholder values, since this `.bin` is what devices actually self-flash. Set those secrets once via the repo's Settings → Secrets and variables → Actions (or `gh secret set NAME`). It can also be re-run manually (`workflow_dispatch`) for an existing tag without re-pushing it.

WiFi credentials don't belong in a public release binary long-term (see "Location & WiFi provisioning" below) — they should live only in NVS via serial provisioning. Getting there is a two-release migration so existing devices aren't stranded: the *first* release after adding NVS-based WiFi provisioning still bakes in the real `WIFI_SSID`/`WIFI_PASS` secrets as before, but `setup()` (`src/main.cpp`, right after `loadWifiConfig()`) migrates them into NVS on first boot if NVS isn't configured yet. Once that release has reached your device(s) (confirm via `CFG GET_STATUS`'s `wifi=1`), flip `WIFI_SSID`/`WIFI_PASS` in `release-firmware.yml` back to `""` and ship the next release — NVS already holds the real credentials by then, so nothing breaks, and from that point on the binary never contains real WiFi credentials again.
...
`connectWiFi()` prefers the NVS-stored credentials but falls back to the compiled-in `WIFI_SSID`/`WIFI_PASS` (`config.h`) when NVS hasn't been provisioned yet. `config.h` is gitignored and normally only a local dev convenience, since the public release build is meant to compile those two fields blank so the distributed `.bin` never contains a real network's credentials — see the two-release migration note under "Firmware auto-update" above for the one release where that isn't yet true. Once the release build does ship blank, a freshly flashed device has no working fallback and must be provisioned once over serial before it can reach WiFi at all; from then on the NVS-stored credentials persist across every future OTA update (NVS isn't touched by an app-partition flash) independently of what's compiled into any given release. `setup()` also runs a one-time migration (`src/main.cpp`, right after `loadWifiConfig()`): if NVS isn't configured yet but a compiled-in `WIFI_SSID` is present, it's copied into NVS immediately, so a device updating from older firmware picks up its working credentials without needing manual serial provisioning.
