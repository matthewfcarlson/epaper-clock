...
// Must match the relay Worker's REPORT_SHARED_SECRET. Unlike WIFI_SSID/PASS,
// the release pipeline *does* compile this in from a repo secret (it isn't
// tied to any one user's network) — an NVS override via CFG SET_REPORT_TOKEN
// still takes priority if set. See "OTA failure reporting" for the tradeoff.
const char *OTA_REPORT_TOKEN = "";
```
...
The device (`reportOtaFailure()`, `src/main.cpp`) POSTs the failure (reason, attempted version, detail, running firmware, timestamp, and the `otaLogBuf` trail above) as JSON to `OTA_REPORT_ENDPOINT` (`config.h` — blank by default, disabling this path entirely), authenticated with `X-Report-Token: <shared secret>` from `activeReportToken()`. Unlike WiFi credentials, that secret's default source *is* the release pipeline: `OTA_REPORT_TOKEN` in `config.h`, compiled in from the `OTA_REPORT_TOKEN` repo secret by `release-firmware.yml` (must match the deployed Worker's `REPORT_SHARED_SECRET`) — reasonable specifically because this secret isn't tied to any one user's network the way WiFi credentials are, and its worst case if leaked is bounded (it can only ever ask the relay's one rate-limited endpoint to do something, never GitHub directly), unlike embedding real WiFi credentials or a GitHub PAT in a public binary. An NVS override still exists and takes priority when set — `CFG SET_REPORT_TOKEN <token>` / `CFG CLEAR_REPORT_TOKEN` (`userReportToken`, `"clockcfg"` NVS namespace, keys `rtok_set`/`rtok`), and the `docs/provision.html` "OTA-failure auto-report" field, wired into the same save/reboot flow as WiFi/location — useful mainly for rotating the secret without reflashing every device. `reportOtaFailure()` is called once per wake right after `checkForFirmwareUpdate()`, regardless of the night-window/throttle gate that function applies to itself — so a failure recorded earlier in the same boot (e.g. by `checkBootHealth()`, before WiFi is up) still gets reported once WiFi is available.
