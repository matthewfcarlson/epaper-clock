...
**Auto-report via a user-provisioned PAT (optional).** For anyone willing to accept the tradeoff the click-to-report flow above exists to avoid, `CFG SET_GH_PAT <token>` / `CFG CLEAR_GH_PAT` (and the `docs/provision.html` "GitHub auto-report" field, wired into the same save/reboot flow as WiFi/location) let the device hold a GitHub personal access token in NVS (`userGithubPat`, `"clockcfg"` namespace, keys `pat_set`/`pat` — same storage pattern as WiFi credentials) and file the issue itself via `reportOtaFailureToGitHub()` (`src/main.cpp`), called once per wake right after `checkForFirmwareUpdate()` regardless of the night-window/throttle gate that function applies to itself (so a failure recorded earlier in the same boot — e.g. by `checkBootHealth()`, before WiFi is up — still gets reported once WiFi is available). A fine-grained token scoped to just "Issues: write" on this one repo is all it needs. This is **not** the same risk shape as embedding a token in the compiled binary: NVS isn't shipped in the public release `.bin`, it's private per-device flash the user writes to themselves. The exposure here is different — NVS isn't encrypted in this project (no flash/NVS encryption configured, same as everywhere else in this codebase), so anyone with physical or USB/serial access to the device can dump flash and read the token in plaintext, and unlike a stolen WiFi password (bounded to LAN access, requires physical proximity), a stolen GitHub token works from anywhere and can act as the user on GitHub until they notice and revoke it — which for a clock nobody's watching logs on could be a long time. `docs/provision.html`'s hint text and this paragraph both say so explicitly; it's opt-in, and the token is never echoed back once saved (same as the WiFi password).

Auto-filing is deduplicated per distinct failure, not per attempt: `OtaHealth` persists a `failureReported` flag (`fail_reported` NVS key) alongside the failure record itself, which `recordFailure()` only resets to `false` when the `(reason, attemptedVersion)` pair actually changes — a retry of the *same* failure (e.g. `download_flash` attempt 2/3 of the same tag, which calls `recordFailure()` again just to refresh the attempt count in `detail`) doesn't get re-reported, and — because this lives in NVS rather than the RTC-memory retry counters (`otaFailedVersion`/`otaFailCount`) — neither does a repeat of the same failure surviving a power cycle that reset those. `reportOtaFailureToGitHub()` checks `!otaHealth.failureReported()` before doing anything and calls `otaHealth.markFailureReported()` only once GitHub's API actually returns 201; a failed POST (no WiFi, API error) leaves it `false` so the next wake retries.
...
- `CFG GET_STATUS` → `>>STATUS configured=0|1 lat=<f> lon=<f> fw=<version> wifi=0|1 owner=<gh_owner> repo=<gh_repo> pat=0|1`
...
- `CFG GET_OTA_STATUS` / `CFG CLEAR_OTA_STATUS`, `CFG GET_OTA_LOG`, `CFG SET_GH_PAT <token>` / `CFG CLEAR_GH_PAT` — see "OTA failure reporting" above
